...
export class ResponsiveState {
    ...
    {
        if ( windowName && this._windows[ windowName ] ) return this._windows[ windowName ].getWidth()
    ...
    public unregisterWindow = ( rw: ResponsiveWindow ) =>
    {
        ...
        {
            if ( this._windows[ rwn ] === rw ) delete (this._windows[rwn])
        }
        window.dispatchEvent( new Event( "resize" ) )
    }
    ...
    {
        let ratio = 1;
        if ( typeof window.screen.systemXDPI !== "undefined" && typeof window.screen.logicalXDPI !== "undefined" && window.screen.systemXDPI > window.screen.logicalXDPI ) ratio = window.screen.systemXDPI / window.screen.logicalXDPI;
        else if (typeof window.devicePixelRatio !== "undefined" ) ratio = window.devicePixelRatio;
    ...
    private orientation_device = (): string =>
    {
        try {
            ...
            } else if ( this.isSMART() || this.isDesktop() ) return "landscape";
            else return null;
        } catch ( e ) {}
        return null;
    }
