...
export class ResponsiveState {
    ...
    private pixel_ratio =():any =>{
        try {
            if (this.test_Is_4k()) {
                return '4k';
            } else if (this.test_Is_retina()) {
                return 'retina';
            } else {
                return '1x';
            }
    ...
    test_Is_retina():any{
        if (!!this.matchMedia) {
            let mq = this.matchMedia("only screen and (min--moz-device-pixel-ratio: 1.3), only screen and (-o-min-device-pixel-ratio: 2.6/2), only screen and (-webkit-min-device-pixel-ratio: 1.3), only screen  and (min-device-pixel-ratio: 1.3), only screen and (min-resolution: 1.3dppx)");
